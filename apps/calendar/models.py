...
class TimeBlockTemplate(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='timeblock_templates')
...
class CalendarShare(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['owner', 'is_active'], name='idx_calshare_owner_active'),
            models.Index(fields=['shared_with', 'is_active'], name='idx_calshare_shared_active'),
            models.Index(fields=['share_token'], name='idx_calshare_token'),
        ]

    def __str__(self):
        target = self.shared_with.email if self.shared_with else f'link:{self.share_token[:8]}'
...
class HabitCompletion(models.Model):
    ...
    count = models.IntegerField(
        default=1,
        help_text='Number of completions for this date.'
    )
