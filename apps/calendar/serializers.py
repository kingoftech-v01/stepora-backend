...
from .models import CalendarEvent, TimeBlock, TimeBlockTemplate, RecurrenceException, CalendarShare, Habit, HabitCompletion
...
class CalendarEventSerializer(serializers.ModelSerializer):
    ...
    dream_id = serializers.UUIDField(source='task.goal.dream.id', read_only=True, allow_null=True, help_text='UUID of the dream the task belongs to.')
    ...
    is_multi_day = serializers.SerializerMethodField(help_text='Whether the event spans multiple days.')
    duration_days = serializers.SerializerMethodField(help_text='Number of days the event spans.')
    recurrence_exceptions = serializers.SerializerMethodField(help_text='List of recurrence exceptions (skips/modifications) for recurring events.')
    display_timezone = serializers.SerializerMethodField(help_text='Effective timezone for display: event_timezone if set, otherwise user home timezone.')
    ...
    class Meta:
        ...
        fields = [
            ...
            'reminder_minutes_before', 'reminders', 'status',
            'category',
            'is_recurring', 'recurrence_rule', 'parent_event',
            'task_title', 'goal_title', 'dream_id', 'dream_title',
            'is_multi_day', 'duration_days', 'recurrence_exceptions',
            'snoozed_until',
            'sync_status', 'last_sync_error',
            'created_at', 'updated_at'
        ]
        read_only_fields = ['id', 'user', 'created_at', 'updated_at', 'is_multi_day', 'duration_days', 'display_timezone', 'snoozed_until', 'sync_status', 'last_sync_error']
        extra_kwargs = {
            ...
            'reminder_minutes_before': {'help_text': 'Minutes before the event to send a reminder (legacy).'},
            'reminders': {'help_text': 'List of reminders: [{minutes_before: int, type: "push"|"email"}].'},
            'status': {'help_text': 'Current status of the event.'},
            'category': {'help_text': 'Event category: meeting, deadline, milestone, habit, social, health, learning, custom.'},
        ...
        return [
            {
                ...
                'modified_start_time': exc.modified_start_time.isoformat() if exc.modified_start_time else None,
                'modified_end_time': exc.modified_end_time.isoformat() if exc.modified_end_time else None,
            }
...
class CalendarEventCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {
            ...
            'all_day': {'help_text': 'Whether this is an all-day event.'},
            'location': {'help_text': 'Location where the event takes place.'},
            'event_timezone': {'help_text': 'Optional timezone override for this event (e.g. America/New_York).'},
            'reminder_minutes_before': {'help_text': 'Minutes before the event to send a reminder (legacy).'},
            'reminders': {'help_text': 'List of reminders: [{minutes_before: int, type: "push"|"email"}].'},
            'category': {'help_text': 'Event category: meeting, deadline, milestone, habit, social, health, learning, custom.'},
            'is_recurring': {'help_text': 'Whether this event repeats on a schedule.'},
        ...
        for i, reminder in enumerate(value):
            if not isinstance(reminder, dict):
                raise serializers.ValidationError(
                    _("Each reminder must be a JSON object (item %(index)d is not)") % {'index': i}
                )
            ...
            if not isinstance(minutes_before, int) or minutes_before < 0 or minutes_before > 40320:
                raise serializers.ValidationError(
    ...
    def validate_recurrence_rule(self, value):
        ...
        if not isinstance(value, dict):
            raise serializers.ValidationError(_("recurrence_rule must be a JSON object"))

        valid_frequencies = ("daily", "weekly", "monthly", "yearly", "custom")
        frequency = value.get("frequency")
        if not frequency:
            raise serializers.ValidationError(_("recurrence_rule requires a 'frequency' field"))
        ...
        if week_of_month is not None:
            if not isinstance(week_of_month, int) or week_of_month < 1 or week_of_month > 5:
                raise serializers.ValidationError(
        ...
        if end_date is not None:
            if not isinstance(end_date, str):
                raise serializers.ValidationError(_("end_date must be an ISO date string or null"))
            try:
                from datetime import datetime as _dt
                _dt.fromisoformat(end_date.replace("Z", "+00:00"))
            except (ValueError, AttributeError):
                raise serializers.ValidationError(_("end_date must be a valid ISO date string"))
...
class TimeBlockSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'start_time', 'end_time', 'is_active', 'focus_block',
            'created_at', 'updated_at'
        ]
        ...
        extra_kwargs = {
            ...
            'focus_block': {'help_text': 'Whether this block is a focus/DND block that suppresses notifications.'},
...
class AcceptScheduleSerializer(serializers.Serializer):
    ...
    class SuggestionItem(serializers.Serializer):
        ...
        suggested_time = serializers.CharField(max_length=5, help_text='Accepted time in HH:MM format.')
...
class BatchScheduleItemSerializer(serializers.Serializer):
    ...
    time = serializers.CharField(max_length=5, help_text='Time to schedule the task (HH:MM).')
...
class CheckConflictsSerializer(serializers.Serializer):
    ...
    start_time = serializers.DateTimeField(help_text='Start date and time to check for conflicts.')
    end_time = serializers.DateTimeField(help_text='End date and time to check for conflicts.')
    exclude_event_id = serializers.UUIDField(required=False, allow_null=True, help_text='Event ID to exclude from conflict check (for reschedule).')
...
class CheckConflictsResponseSerializer(serializers.Serializer):
    ...
    has_conflicts = serializers.BooleanField(help_text='Whether there are any conflicts.')
    conflicts = serializers.ListField(help_text='List of conflicting events and time blocks.')
...
class TimeBlockTemplateSerializer(serializers.ModelSerializer):
    ...
    block_count = serializers.SerializerMethodField(help_text='Number of blocks in this template.')

    class Meta:
        model = TimeBlockTemplate
        fields = [
            'id', 'user', 'name', 'description', 'blocks',
            'is_preset', 'block_count', 'created_at', 'updated_at',
        ]
        ...
        extra_kwargs = {
            ...
            'blocks': {'help_text': 'Array of {block_type, day_of_week, start_time, end_time}.'},
    ...
    def validate_blocks(self, value):
        ...
        if len(value) == 0:
            raise serializers.ValidationError(_("blocks must contain at least one block"))
        valid_types = ('work', 'personal', 'family', 'exercise', 'blocked')
        for i, block in enumerate(value):
            if not isinstance(block, dict):
                raise serializers.ValidationError(
                    _("Each block must be a JSON object (item %(index)d is not)") % {'index': i}
                )
            for field in ('block_type', 'day_of_week', 'start_time', 'end_time'):
                if field not in block:
                    raise serializers.ValidationError(
                        _("Block %(index)d is missing required field '%(field)s'") % {'index': i, 'field': field}
                    )
            if block['block_type'] not in valid_types:
                raise serializers.ValidationError(
                    _("Block %(index)d has invalid block_type '%(type)s'") % {'index': i, 'type': block['block_type']}
                )
...
class SaveCurrentTemplateSerializer(serializers.Serializer):
    ...
    description = serializers.CharField(required=False, default='', help_text='Optional description.')
...
class HeatmapDaySerializer(serializers.Serializer):
    ...
    tasks_completed = serializers.IntegerField(help_text='Number of tasks completed on this day.')
    tasks_total = serializers.IntegerField(help_text='Total number of tasks scheduled for this day.')
    events_count = serializers.IntegerField(help_text='Number of calendar events on this day.')
    focus_minutes = serializers.IntegerField(help_text='Total focus session minutes on this day.')
    productivity_score = serializers.FloatField(help_text='Weighted productivity score (0.0-1.0).')
...
class RecurrenceExceptionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {
            ...
            'parent_event': {'help_text': 'The recurring event this exception applies to.'},
            'original_date': {'help_text': 'The original occurrence date.'},
            'skip_occurrence': {'help_text': 'Whether to skip this occurrence entirely.'},
...
class ModifyOccurrenceSerializer(serializers.Serializer):
    ...
    start_time = serializers.DateTimeField(
        ...
        help_text='New start time for this occurrence.'
    )
    end_time = serializers.DateTimeField(
        required=False, allow_null=True,
        help_text='New end time for this occurrence.'
    )
...
class CalendarShareSerializer(serializers.ModelSerializer):
    ...
    owner_name = serializers.CharField(source='owner.display_name', read_only=True, allow_blank=True)
    owner_avatar = serializers.SerializerMethodField()
    shared_with_name = serializers.CharField(source='shared_with.display_name', read_only=True, allow_blank=True, allow_null=True)
    shared_with_avatar = serializers.SerializerMethodField()

    class Meta:
        model = CalendarShare
        fields = [
            'id', 'owner', 'shared_with', 'permission', 'share_token',
            'is_active', 'created_at',
            'owner_name', 'owner_avatar',
            'shared_with_name', 'shared_with_avatar',
        ]
        read_only_fields = [
            'id', 'owner', 'shared_with', 'share_token',
            'is_active', 'created_at',
        ]
        extra_kwargs = {
    ...
    def get_owner_avatar(self, obj):
        if obj.owner and hasattr(obj.owner, 'avatar') and obj.owner.avatar:
            return obj.owner.avatar.url if hasattr(obj.owner.avatar, 'url') else str(obj.owner.avatar)
        return ''

    def get_shared_with_avatar(self, obj):
        if obj.shared_with and hasattr(obj.shared_with, 'avatar') and obj.shared_with.avatar:
            return obj.shared_with.avatar.url if hasattr(obj.shared_with.avatar, 'url') else str(obj.shared_with.avatar)
        return ''
...
class HabitSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'custom_days', 'target_per_day', 'color', 'icon',
            'is_active', 'streak_current', 'streak_best',
            'completions_today',
            'created_at', 'updated_at',
        ]
        read_only_fields = ['id', 'user', 'streak_current', 'streak_best', 'created_at', 'updated_at']
        extra_kwargs = {
            ...
            'custom_days': {'help_text': 'Days of the week (0-6) for custom frequency.'},
    ...
    def validate_color(self, value):
        ...
        if value and not re.match(r'^#[0-9A-Fa-f]{6}$', value):
            raise serializers.ValidationError(_("color must be a valid hex color code (e.g. #8B5CF6)"))
        return value

    def validate_target_per_day(self, value):
        if value < 1 or value > 100:
            raise serializers.ValidationError(_("target_per_day must be between 1 and 100"))
        return value
...
class HabitCompletionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'habit', 'completed_at', 'date', 'count', 'note',
            'habit_name', 'habit_color', 'habit_icon',
        ]
