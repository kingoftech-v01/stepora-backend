"""
Models for AI Coaching conversations.

Standalone models with new database tables, separate from apps.chat.
AI conversations have no target_user or buddy_pairing fields.
"""
...
class AIConversation(models.Model):
    """AI coaching conversation session."""
...
class ConversationBranch(models.Model):
    """A branch point in an AI conversation, allowing exploration of alternate paths."""

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    conversation = models.ForeignKey(
        AIConversation, on_delete=models.CASCADE, related_name="branches"
    )
...
class ConversationSummary(models.Model):
    """Summarized AI conversation for long-term context."""

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    conversation = models.ForeignKey(
        AIConversation, on_delete=models.CASCADE, related_name="summaries"
    )
    ...
    start_message = models.ForeignKey(
        AIMessage, on_delete=models.CASCADE, related_name="summary_starts"
    )
    end_message = models.ForeignKey(
        AIMessage, on_delete=models.CASCADE, related_name="summary_ends"
    )
...
class ConversationTemplate(models.Model):
    """Pre-built conversation templates for AI coaching use cases."""
...
class ChatMemory(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="ai_chat_memories"
    )
