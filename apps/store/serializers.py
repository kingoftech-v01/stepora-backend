...
class StoreItemSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {
            ...
            "available_until": {
                "help_text": "End date when item is no longer available."
            },
...
class UserInventorySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {
            ...
            "stripe_payment_intent_id": {
                "help_text": "Stripe PaymentIntent ID for the purchase."
            },
...
class PurchaseSerializer(serializers.Serializer):
    ...
    item_id = serializers.UUIDField(help_text="UUID of the store item to purchase.")
    ...
    def validate(self, attrs):
        ...
        if request and hasattr(request, "user") and request.user.is_authenticated:
            already_owned = UserInventory.objects.filter(
                user=request.user, item_id=attrs["item_id"]
            ).exists()
            if already_owned:
                raise serializers.ValidationError(
...
class PurchaseConfirmSerializer(serializers.Serializer):
    ...
    item_id = serializers.UUIDField(help_text="UUID of the store item being purchased.")
    ...
    def validate(self, attrs):
        ...
        if request and request.user.is_authenticated:
            if Wishlist.objects.filter(
                ...
                raise serializers.ValidationError(
                    {"item_id": "Item already in your wishlist."}
                )
        return attrs
...
class RefundRequestSerializer(serializers.Serializer):
    ...
    inventory_id = serializers.UUIDField(
        help_text="UUID of the inventory entry to refund."
    )
...
class RefundRequestDisplaySerializer(serializers.ModelSerializer):
    ...
    item_name = serializers.CharField(
        source="inventory_entry.item.name",
        read_only=True,
