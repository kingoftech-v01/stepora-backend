...
class TestUserInventoryModel:
    ...
    def test_inventory_equipped_str(self, store_user, gold_frame):
        ...
        UserInventory.objects.filter(user=store_user, item=gold_frame).exclude(
            id=entry.id
        ).delete()
    ...
    def test_unique_together_user_item(
        self, store_user, gold_frame, owned_gold_frame, db
    ):
...
class TestStoreServiceConfirmPurchase:
    ...
    def test_already_owned_raises_error(
        ...
        with pytest.raises(ItemAlreadyOwnedError):
            StoreService.confirm_purchase(store_user, gold_frame, "pi_test_duplicate")

    def test_failed_payment_raises_error(self, store_user, gold_frame):
        """Test that a non-succeeded payment intent raises error."""
        with patch(
            "apps.store.services.stripe.PaymentIntent.retrieve"
        ) as mock_retrieve:
            mock_intent = MagicMock()
            mock_intent.status = "requires_payment_method"
            mock_retrieve.return_value = mock_intent
            with pytest.raises(
                PaymentVerificationError, match="Payment has not been completed"
            ):
                StoreService.confirm_purchase(store_user, gold_frame, "pi_test_failed")
...
class TestStoreServiceInventory:
    ...
    def test_equip_other_users_item_raises_error(self, other_user, owned_gold_frame):
...
class TestSerializers:
    ...
    def test_purchase_confirm_serializer_valid(self, gold_frame):
        ...
        serializer = PurchaseConfirmSerializer(
            data={
                "item_id": str(gold_frame.id),
