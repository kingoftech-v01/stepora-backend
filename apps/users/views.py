...
from core.throttles import ExportRateThrottle, TwoFactorRateThrottle, AIMotivationRateThrottle
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def export_data(self, request):
        ...
        try:
            ...
            for conv in Conversation.objects.filter(user=user).order_by('-updated_at')[:100]:
        ...
        try:
            ...
            for session in FocusSession.objects.filter(user=user).order_by('-started_at')[:200]:
        ...
        if export_format == 'csv':
            ...
            writer.writerow([
                'dream_title', 'dream_category', 'dream_status', 'dream_progress',
                'goal_title', 'goal_status', 'goal_progress',
                'task_title', 'task_status', 'task_duration_mins', 'task_completed_at',
            ])
            for dream in data['dreams']:
                if not dream['goals']:
                    writer.writerow([
                        dream['title'], dream['category'], dream['status'],
                ...
                for goal in dream['goals']:
                    if not goal['tasks']:
                        writer.writerow([
                            dream['title'], dream['category'], dream['status'],
                            dream['progress_percentage'],
                            goal['title'], goal['status'], goal['progress_percentage'],
                            '', '', '', '',
                        ])
    ...
    def streak_details(self, request):
        ...
        all_activities = DailyActivity.objects.filter(
            user=user, tasks_completed__gt=0
        ...
        if user.streak_days > 0 and (not yesterday_activity or yesterday_activity.tasks_completed == 0):
            streak_frozen = True
    ...
    @extend_schema(summary="Get achievements", description="List all achievements with unlock status and progress", tags=["Users"], responses={200: dict, 404: OpenApiResponse(description='Resource not found.')})
    @action(detail=False, methods=['get'])
    ...
    def _compute_achievement_progress(self, user):
        ...
        try:
            from apps.circles.models import CircleMembership
            progress['circles_joined'] = CircleMembership.objects.filter(user=user).count()
        except Exception:
        ...
        try:
            ...
            progress['likes_received'] = PostLike.objects.filter(post__author=user).count()
        except Exception:
    ...
    def energy_profile(self, request):
        ...
        if pattern not in valid_patterns:
            return Response(
                {'error': _('energy_pattern must be one of: morning_person, night_owl, steady')},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        def validate_hour_ranges(ranges, field_name):
            ...
            for r in ranges:
                if not isinstance(r, dict):
                    return _('Each range in %(field)s must be a JSON object') % {'field': field_name}
                start = r.get('start')
                end = r.get('end')
                if not isinstance(start, int) or not isinstance(end, int):
                    return _('start and end must be integers in %(field)s') % {'field': field_name}
                if start < 0 or start > 23 or end < 0 or end > 23:
                    return _('Hours must be between 0 and 23 in %(field)s') % {'field': field_name}
                if start >= end:
                    return _('start must be less than end in %(field)s') % {'field': field_name}
            return None
    ...
    def notification_timing(self, request):
        ...
        if request.method == 'PUT':
            ...
            if (
                not isinstance(qh_start, int) or qh_start < 0 or qh_start > 23
                or not isinstance(qh_end, int) or qh_end < 0 or qh_end > 23
            ):
                return Response(
        ...
        activities = DailyActivity.objects.filter(
            user=user, date__gte=thirty_days_ago.date(),
        ).order_by('date')

        # Build activity summary by day of week
        day_activity = defaultdict(lambda: {'tasks': 0, 'xp': 0, 'minutes': 0, 'count': 0})
        ...
        type_engagement = defaultdict(lambda: {'sent': 0, 'read': 0, 'opened': 0, 'hours': []})
        ...
        if not notification_types:
            notification_types = [
                ...
                'check_in', 'daily_summary', 'weekly_report',
            ]
        ...
        try:
            ...
            return Response(
                {'error': _('AI optimization temporarily unavailable. Please try again later.')},
    ...
    def personality_quiz(self, request):
        ...
        DIMENSION_NAMES = ['visionary', 'achiever', 'explorer', 'collaborator', 'strategist']
        scores = [0, 0, 0, 0, 0]
    ...
    @action(detail=False, methods=['get'], url_path='daily-quote', permission_classes=[IsAuthenticated])
    def daily_quote(self, request):
        ...
        quotes = [
            ...
            {"quote": "Success is not final, failure is not fatal: it is the courage to continue that counts.", "author": "Winston Churchill", "category": "courage"},
            {"quote": "In the middle of every difficulty lies opportunity.", "author": "Albert Einstein", "category": "perseverance"},
            {"quote": "What you get by achieving your goals is not as important as what you become by achieving your goals.", "author": "Zig Ziglar", "category": "growth"},
            ...
            {"quote": "You are never too old to set another goal or to dream a new dream.", "author": "C.S. Lewis", "category": "dreams"},
            {"quote": "The only limit to our realization of tomorrow will be our doubts of today.", "author": "Franklin D. Roosevelt", "category": "self-belief"},
            {"quote": "Act as if what you do makes a difference. It does.", "author": "William James", "category": "action"},
            {"quote": "What lies behind us and what lies before us are tiny matters compared to what lies within us.", "author": "Ralph Waldo Emerson", "category": "self-belief"},
            {"quote": "The best time to plant a tree was 20 years ago. The second best time is now.", "author": "Chinese Proverb", "category": "action"},
            {"quote": "Your limitation—it's only your imagination.", "author": "Unknown", "category": "self-belief"},
            {"quote": "Great things never come from comfort zones.", "author": "Unknown", "category": "courage"},
            {"quote": "Dream it. Wish it. Do it.", "author": "Unknown", "category": "action"},
            {"quote": "The harder you work for something, the greater you'll feel when you achieve it.", "author": "Unknown", "category": "perseverance"},
            {"quote": "Don't stop when you're tired. Stop when you're done.", "author": "Unknown", "category": "perseverance"},
            {"quote": "Wake up with determination. Go to bed with satisfaction.", "author": "Unknown", "category": "motivation"},
            {"quote": "Little things make big days.", "author": "Unknown", "category": "growth"},
            {"quote": "It's going to be hard, but hard does not mean impossible.", "author": "Unknown", "category": "perseverance"},
            ...
            {"quote": "Go the extra mile. It's never crowded there.", "author": "Dr. Wayne D. Dyer", "category": "action"},
            {"quote": "Keep your face always toward the sunshine—and shadows will fall behind you.", "author": "Walt Whitman", "category": "motivation"},
            {"quote": "You don't have to be great to start, but you have to start to be great.", "author": "Zig Ziglar", "category": "action"},
            {"quote": "A champion is defined not by their wins but by how they can recover when they fall.", "author": "Serena Williams", "category": "courage"},
            {"quote": "The mind is everything. What you think you become.", "author": "Buddha", "category": "self-belief"},
            {"quote": "Strive not to be a success, but rather to be of value.", "author": "Albert Einstein", "category": "growth"},
        ]
        ...
        request={
            'application/json': {
                ...
                'properties': {
                    'mood': {
                        'type': 'string',
                        'enum': ['excited', 'motivated', 'neutral', 'tired', 'frustrated', 'anxious', 'sad'],
                    },
                },
                'required': ['mood'],
            },
    ...
    def motivation(self, request):
        ...
        valid_moods = ['excited', 'motivated', 'neutral', 'tired', 'frustrated', 'anxious', 'sad']
        ...
        active_dreams = Dream.objects.filter(user=user, status='active').order_by('-updated_at')[:5]
        progress_lines = []
        ...
        ).select_related('goal__dream').order_by('-completed_at')[:5]
        completion_lines = []
        for task in recent_tasks:
            dream_title = task.goal.dream.title if task.goal and task.goal.dream else 'Unknown'
            completion_lines.append(f"- Completed \"{task.title}\" (dream: {dream_title})")
    ...
    @action(detail=False, methods=['get'], url_path='morning-briefing', permission_classes=[IsAuthenticated])
    def morning_briefing(self, request):
        ...
        from apps.dreams.models import Dream, Task, FocusSession
        ...
        tasks_today_qs = Task.objects.filter(
            goal__dream__user=user,
            status='pending',
        ).filter(
        ...
        ).select_related('goal__dream').order_by('scheduled_date', 'scheduled_time', 'order')[:10]
        ...
        for t in tasks_today_qs:
            ...
            priority = 'high' if dream_priority >= 3 else ('medium' if dream_priority >= 2 else 'low')
        ...
        if streak_days >= 100:
            streak_message = f"Legendary! {streak_days} day streak \U0001f525\U0001f525\U0001f525"
        elif streak_days >= 30:
            streak_message = f"Unstoppable! {streak_days} day streak \U0001f525\U0001f525"
        elif streak_days >= 7:
        ...
        active_dreams = list(
            ...
            .values('id', 'title', 'progress_percentage')
        )
        ...
        if active_dreams:
            ...
            next_task_obj = Task.objects.filter(
                goal__dream__id=spotlight_dream['id'],
                status='pending',
            ).order_by('scheduled_date', 'order').first()
        ...
        stats_yesterday = {
            'tasks_completed': yesterday_activity.tasks_completed if yesterday_activity else 0,
    ...
    def weekly_report(self, request):
        ...
        from apps.dreams.models import Dream, Task, Goal, FocusSession, DreamProgressSnapshot
        from core.permissions import CanUseAI
        from integrations.openai_service import OpenAIService
        from core.exceptions import OpenAIError
    ...
    def check_in(self, request):
        ...
        for t in pending_tasks_qs:
            ...
            if t.deadline_date:
                ...
                due = str(t.scheduled_date.date()) if hasattr(t.scheduled_date, 'date') else str(t.scheduled_date)
    ...
    def productivity_insights(self, request):
        ...
        from datetime import timedelta as td
        from apps.dreams.models import Task, FocusSession
        from integrations.openai_service import OpenAIService
        from core.exceptions import OpenAIError
        ...
        django_day_names = {1: 'Sunday', 2: 'Monday', 3: 'Tuesday', 4: 'Wednesday',
