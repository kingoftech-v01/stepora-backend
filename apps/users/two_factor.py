...
def _generate_backup_codes(count=BACKUP_CODE_COUNT):
    ...
    return [secrets.token_hex(BACKUP_CODE_LENGTH // 2).upper() for _ in range(count)]
...
class TwoFactorVerifyView(APIView):
    ...
    def post(self, request):
        ...
        secret = (
            user.totp_secret
        )  # EncryptedCharField (holds both pending and active secrets)
        ...
        if is_pending:
            ...
            user.save(update_fields=["totp_enabled", "backup_codes", "app_prefs"])
...
class TwoFactorDisableView(APIView):
    ...
    def post(self, request):
        ...
        user.save(
            update_fields=["totp_enabled", "totp_secret", "backup_codes", "app_prefs"]
        )
    ...
    @extend_schema(
        ...
        responses={
            200: inline_serializer(
                ...
                fields={
                    "backup_codes": drf_serializers.ListField(
