...
class TestSearchService:
    ...
    def test_global_search_default_types(self, user):
        with patch.object(SearchService, 'search_dreams', return_value=['d1']), \
             patch.object(SearchService, 'search_goals', return_value=['g1']), \
             patch.object(SearchService, 'search_tasks', return_value=[]), \
             patch.object(SearchService, 'search_messages', return_value=[]), \
             patch.object(SearchService, 'search_users', return_value=[]), \
             patch.object(SearchService, 'search_calendar', return_value=[]):
...
class TestGlobalSearchView:
    ...
    def test_search_multiple_type_filter(self, mock_search, authenticated_client, user):
        ...
        mock_search.assert_called_once_with(user, 'test', types=['dreams', 'users'], limit=10)
