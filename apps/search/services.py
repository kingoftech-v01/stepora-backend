...
class SearchService:
    ...
    def search_users(query, limit=MAX_RESULTS):
        ...
        try:
            ...
            s = s.query(
                ESQ("match", display_name={"query": query, "fuzziness": "AUTO"})
            )
            s = s[:limit]
            response = s.execute()
            return [hit.meta.id for hit in response]
    ...
    def global_search(user, query, types=None, limit=10):
        ...
        search_map = {
            ...
            "circles": lambda: SearchService.search_circle_posts(
                query, user=user, limit=limit
            ),
            "circle_challenges": lambda: SearchService.search_circle_challenges(
                query, user=user, limit=limit
            ),
            "activity_comments": lambda: SearchService.search_activity_comments(
