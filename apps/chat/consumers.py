"""
WebSocket consumers for AI coach chat and buddy-to-buddy chat.

Merged from apps.conversations.consumers and apps.buddies.consumers.
...
class AIChatConsumer(
    ...
    Rejects buddy_chat conversations -- those use BuddyChatConsumer instead.
    """
    ...
    async def _setup_authenticated_inner(self):
        """Verify access, reject buddy_chat, join room, send confirmation."""
        conversation = await self._load_and_verify_conversation()
        if not conversation:
            await self.close(code=4003)
            return

        # Reject buddy_chat conversations -- they use BuddyChatConsumer
        if conversation.conversation_type == "buddy_chat":
            await self.send_error(
                "Buddy chat conversations should use ws/buddy-chat/<pairing_id>/"
            )
    ...
    def _load_and_verify_conversation(self):
        try:
            conversation = Conversation.objects.select_related("user", "dream").get(
...
class BuddyChatConsumer(
    ...
    def _load_and_verify_pairing(self):
        ...
        try:
            pairing = BuddyPairing.objects.select_related("user1", "user2").get(
                id=self.pairing_id, status="active"
            )
    ...
    def _mark_conversation_read(self):
        ...
        last_msg = (
            Message.objects.filter(conversation=self._conversation_cache)
            .order_by("-created_at")
            .first()
        )
        if last_msg:
    ...
    def _send_push_notification(self, content):
        ...
        try:
            ...
            devices = UserDevice.objects.filter(user=self._partner, is_active=True)
