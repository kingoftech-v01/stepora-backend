...
AI chat consumer is in apps.ai.consumers.
...
from .models import ChatConversation, ChatMessage, MessageReadStatus
...
class BuddyChatConsumer(
    ...
    def _save_message(self, content):
        ...
        ChatConversation.objects.filter(id=self._conversation_cache.id).update(
    ...
    def _mark_conversation_read(self):
        last_msg = (
            ChatMessage.objects.filter(conversation=self._conversation_cache)
            .order_by("-created_at")
            .first()
        )
        if last_msg:
