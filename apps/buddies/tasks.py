...
def send_buddy_checkin_reminders():
    ...
    stale_pairings = BuddyPairing.objects.filter(
        status='active',
    ).filter(
        Q(last_encouragement_at__lt=threshold) | Q(last_encouragement_at__isnull=True)
    ...
    for pairing in stale_pairings:
        for user in [pairing.user1, pairing.user2]:
            ...
            try:
                ...
                logger.exception(
                    "Failed to send buddy checkin reminder for pairing %s",
                    pairing.id
                )
