...
from .models import BuddyEncouragement, BuddyPairing
...
class TestBuddyCurrentAction:
    ...
    def test_current_with_active_pairing(
        self, authenticated_client, buddy_pairing, user, other_user
    ):
...
class TestBuddyProgressAction:
    ...
    def test_progress_returns_comparison(
        self, authenticated_client, buddy_pairing, user, other_user
    ):
    ...
    def test_progress_inactive_pairing_not_found(
...
class TestBuddyFindMatchAction:
    ...
    def test_find_match_shared_interests(
        self,
        authenticated_client,
        candidate_user,
        user_gamification,
...
class TestBuddyPairAction:
    ...
    def test_pair_success(self, authenticated_client, other_user, user):
        response = authenticated_client.post(
            f"{BASE_URL}pair/", {"partner_id": str(other_user.id)}
        )
    ...
    def test_pair_self_pairing(self, authenticated_client, user):
        response = authenticated_client.post(
            f"{BASE_URL}pair/", {"partner_id": str(user.id)}
        )
        assert response.status_code == status.HTTP_400_BAD_REQUEST
    ...
    def test_pair_already_paired(self, authenticated_client, buddy_pairing, third_user):
        response = authenticated_client.post(
            f"{BASE_URL}pair/", {"partner_id": str(third_user.id)}
        )
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert "already have" in response.data["error"]
    ...
    def test_pair_partner_not_found(self, authenticated_client):
        fake_id = uuid.uuid4()
        response = authenticated_client.post(
            f"{BASE_URL}pair/", {"partner_id": str(fake_id)}
        )
        assert response.status_code == status.HTTP_404_NOT_FOUND
        assert "not found" in response.data["error"]

    def test_pair_partner_already_paired(
    ...
    def test_pair_calculates_compatibility(
        self, authenticated_client, other_user, user
    ):
...
class TestBuddyAcceptAction:
    ...
    def test_accept_only_user2_can_accept(
        self, api_client, pending_pairing, other_user
    ):
...
class TestBuddyRejectAction:
    ...
    def test_reject_only_user2_can_reject(
        self, api_client, pending_pairing, other_user
    ):
        api_client.force_authenticate(user=other_user)
...
class TestBuddyEncourageAction:
    ...
    def test_encourage_streak_increment_next_day(
        self, authenticated_client, buddy_pairing
    ):
    ...
    def test_encourage_streak_same_day_no_change(
        self, authenticated_client, buddy_pairing
    ):
    ...
    def test_encourage_creates_notification(
        self, authenticated_client, buddy_pairing, user, other_user
    ):
        ...
        notif = Notification.objects.filter(
            user=other_user, notification_type="buddy"
        ).first()
...
class TestBuddyHistoryAction:
    ...
    def test_history_returns_past_pairings(
        self, authenticated_client, user, other_user
    ):
        now = timezone.now()
        p1 = BuddyPairing.objects.create(
    ...
    def test_history_includes_active_pairings(
        self, authenticated_client, buddy_pairing
    ):
    ...
    def test_history_includes_encouragement_count(
        self, authenticated_client, buddy_pairing, user
    ):
        BuddyEncouragement.objects.create(
            pairing=buddy_pairing, sender=user, message="1"
        )
        BuddyEncouragement.objects.create(
            pairing=buddy_pairing, sender=user, message="2"
        )
        response = authenticated_client.get(f"{BASE_URL}history/")
        assert response.status_code == status.HTTP_200_OK
        assert response.data["pairings"][0]["encouragementCount"] == 2

    def test_history_includes_duration_days(
        self, authenticated_client, user, other_user
    ):
    ...
    def test_history_duration_null_when_not_ended(
        self, authenticated_client, buddy_pairing
    ):
        response = authenticated_client.get(f"{BASE_URL}history/")
        assert response.status_code == status.HTTP_200_OK
