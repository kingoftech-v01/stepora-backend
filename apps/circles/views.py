...
class CircleViewSet(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if not instance.is_public:
            if not CircleMembership.objects.filter(
                circle=instance, user=request.user
            ).exists():
                return Response(
    ...
    @extend_schema(
        ...
        responses={
            ...
            400: OpenApiResponse(
                description="Circle is full or user already a member."
            ),
            403: OpenApiResponse(description="Subscription required."),
            404: OpenApiResponse(description="Resource not found."),
        },
    ...
    def join(self, request, pk=None):
        ...
        CircleMembership.objects.create(circle=circle, user=request.user, role="member")
    ...
    def leave(self, request, pk=None):
        ...
        try:
            membership = CircleMembership.objects.get(circle=circle, user=request.user)
        except CircleMembership.DoesNotExist:
            return Response(
        ...
        if membership.role == "admin":
            other_admins = (
                CircleMembership.objects.filter(circle=circle, role="admin")
        ...
        membership.delete()
        return Response(
            {"message": _("Successfully left %(name)s.") % {"name": circle.name}}
        )
    ...
    def feed(self, request, pk=None):
        ...
        posts = (
            CirclePost.objects.filter(circle=circle)
    ...
    def challenges(self, request, pk=None):
        ...
        challenges = CircleChallenge.objects.filter(
            circle=circle, status__in=["upcoming", "active"]
        ).order_by("-start_date")

        serializer = CircleChallengeSerializer(
    ...
    @extend_schema(
        ...
        responses={
            ...
            400: OpenApiResponse(description="Validation error."),
            403: OpenApiResponse(
                description="Only admins/moderators can create challenges."
            ),
            404: OpenApiResponse(description="Resource not found."),
        },
        tags=["Circles"],
    ...
    @action(
        detail=True, methods=["put"], url_path=r"posts/(?P<post_id>[0-9a-f-]+)/edit"
    )
    def edit_post(self, request, pk=None, post_id=None):
        ...
        try:
            ...
            return Response(
                {"error": _("Post not found.")}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    @action(
        detail=True,
        methods=["delete"],
        url_path=r"posts/(?P<post_id>[0-9a-f-]+)/delete",
    )
    ...
    @action(
        detail=True, methods=["post"], url_path=r"posts/(?P<post_id>[0-9a-f-]+)/react"
    )
    def react_to_post(self, request, pk=None, post_id=None):
        ...
        if created:
            return Response(
                {"message": _("Reaction added.")}, status=status.HTTP_201_CREATED
            )
    ...
    @action(
        detail=True,
        methods=["delete"],
        url_path=r"posts/(?P<post_id>[0-9a-f-]+)/unreact",
    )
    def unreact_to_post(self, request, pk=None, post_id=None):
        ...
        deleted_count, _detail = PostReaction.objects.filter(
            post=post, user=request.user
        ).delete()

        if deleted_count == 0:
            return Response(
                {"error": _("No reaction found.")}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    @action(
        detail=True, methods=["post"], url_path=r"posts/(?P<post_id>[0-9a-f-]+)/vote"
    )
    def vote_on_poll(self, request, pk=None, post_id=None):
        ...
        try:
            ...
            return Response(
                {"error": _("This post does not have a poll.")},
    ...
    @action(
        detail=True,
        methods=["post"],
        url_path=r"members/(?P<member_id>[0-9a-f-]+)/promote",
    )
    def promote_member(self, request, pk=None, member_id=None):
        ...
        try:
            target_membership = CircleMembership.objects.get(
                id=member_id, circle=circle
            )
        except CircleMembership.DoesNotExist:
            return Response(
                {"error": _("Member not found.")}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        return Response(
            {
                "message": _("%(name)s promoted to moderator.")
                % {"name": target_membership.user.display_name or _("User")}
            }
        )
    ...
    @action(
        detail=True,
        methods=["post"],
        url_path=r"members/(?P<member_id>[0-9a-f-]+)/demote",
    )
    def demote_member(self, request, pk=None, member_id=None):
        ...
        return Response(
            {
                "message": _("%(name)s demoted to member.")
                % {"name": target_membership.user.display_name or _("User")}
            }
        )
    ...
    @action(
        detail=True,
        methods=["delete"],
        url_path=r"members/(?P<member_id>[0-9a-f-]+)/remove",
    )
    ...
    @extend_schema(
        ...
        responses={
            ...
            400: OpenApiResponse(
                description="User already a member or already invited."
            ),
            403: OpenApiResponse(
                description="Not an admin/moderator or subscription required."
            ),
            404: OpenApiResponse(description="Resource not found."),
        },
        tags=["Circles"],
    ...
    def invitations(self, request, pk=None):
        ...
        invitations = (
            CircleInvitation.objects.filter(circle=circle, status="pending")
    ...
    @action(
        detail=True,
        methods=["post"],
        url_path=r"challenges/(?P<challenge_id>[0-9a-f-]+)/progress",
    )
    def submit_progress(self, request, pk=None, challenge_id=None):
        ...
        try:
            ...
            return Response(
                {"error": _("Challenge not found.")}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    @action(
        detail=True,
        methods=["get"],
        url_path=r"challenges/(?P<challenge_id>[0-9a-f-]+)/leaderboard",
    )
    ...
    def call_start(self, request, pk=None):
        ...
        if active_call:
            return Response(
                {
                    "error": _("A call is already active in this circle."),
        ...
        try:
            ...
            for token in tokens:
                try:
                    fcm.send_to_token(
                        token=token,
                        title=_("%(name)s started a %(type)s call")
                        % {"name": caller_name, "type": call.call_type},
...
class ChallengeViewSet(viewsets.GenericViewSet):
    ...
    def join(self, request, pk=None):
        ...
        if not CircleMembership.objects.filter(
            circle=challenge.circle, user=request.user
        ).exists():
            return Response(
                {
                    "error": _(
                        "You must be a member of the circle to join this challenge."
                    )
                },
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        return Response(
            {
                "message": _("Successfully joined challenge: %(title)s.")
