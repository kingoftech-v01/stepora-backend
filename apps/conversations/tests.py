...
class TestMessageModel:
    ...
    def test_message_str_truncates_long_content(self, db, conversation):
        ...
        long_content = "A" * 100
        message = Message.objects.create(
            conversation=conversation, role="assistant", content=long_content
        )
        result = str(message)
...
class TestConversationViewSet:
    ...
    def test_create_planning_conversation_requires_dream(
    ...
    def test_list_messages(self, authenticated_client, conversation):
        ...
        # Create messages
        Message.objects.create(
            conversation=conversation, role="user", content="Message 1"
        )
        Message.objects.create(
            conversation=conversation, role="assistant", content="Message 2"
        )
    ...
    def test_send_message(self, authenticated_client, conversation, mock_openai):
        ...
        data = {"content": "Hello, AI assistant!"}
    ...
    def test_cannot_access_other_user_conversation(
        ...
        other_conversation = Conversation.objects.create(
            user=other_user, conversation_type="general"
        )
...
class TestChatConsumer:
    ...
    async def test_send_message(self, db, user, conversation):
        ...
        with patch("apps.conversations.consumers.OpenAIService") as mock_service:
            ...
            await communicator.send_json_to({"type": "message", "message": "Hello AI"})
    ...
    async def test_empty_message_rejected(self, db, user, conversation):
        ...
        await communicator.send_json_to(
            {"type": "message", "message": "   "}  # Whitespace only
        )
...
class TestConversationTypes:
    ...
    def test_motivation_conversation(self, db, user):
        ...
        conversation = Conversation.objects.create(
            user=user, conversation_type="motivation"
        )
