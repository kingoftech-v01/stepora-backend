...
class ModerationResult:
    ...
    detection_source: str = ''  # 'openai_api', 'jailbreak_pattern', 'dream_content', 'roleplay'
...
JAILBREAK_PATTERNS = [
    ...
    re.compile(r'(encode|translate)\s+(your\s+)?(response|answer|output)\s+(in|to)\s+(base64|hex|binary)', re.IGNORECASE),
...
# ---------------------------------------------------------------------------
ROLEPLAY_PATTERNS = [
    re.compile(r'(pretend|imagine|suppose|assume)\s+(you\s+are|you\'re|to\s+be)\s+', re.IGNORECASE),
    re.compile(r'(pr[eé]tends?|imagine)\s+(que\s+)?tu\s+(es|sois)\s+', re.IGNORECASE),  # FR
    re.compile(r'role\s*-?\s*play\s+as\b', re.IGNORECASE),
    re.compile(r'act\s+as\s+(if\s+you\s+are|though\s+you|a)\s+(?!motivated|focused|disciplined)', re.IGNORECASE),
    re.compile(
        r'you\s+are\s+(my|a)\s+(girlfriend|boyfriend|therapist|doctor|lawyer|hacker|evil|villain)',
        re.IGNORECASE,
    ),
    re.compile(r'adopt\s+the\s+(role|persona|character|identity)\s+of\b', re.IGNORECASE),
    ...
    re.compile(r'(sois|deviens)\s+(mon|ma|un|une)\s+', re.IGNORECASE),  # FR: "sois ma copine"
]
...
HARMFUL_DREAM_PATTERNS_VIOLENCE = [
    ...
    re.compile(r'\b(revenge|vengeance|get\s+back\s+at|make\s+.*\s+pay|destroy\s+.*\s+life)\b', re.IGNORECASE),
]
...
HARMFUL_DREAM_PATTERNS_COERCION = [
    ...
    re.compile(r'\bstalk(ing)?\b.*\b(someone|person|him|her|them|my\s+ex)\b', re.IGNORECASE),
    re.compile(
...
HARMFUL_DREAM_PATTERNS_SELF_HARM = [
    re.compile(r'\b(suicide|kill\s+myself|end\s+my\s+life|self[- ]?harm|cut\s+myself)\b', re.IGNORECASE),
    re.compile(r'\b(me\s+suicider|mettre\s+fin\s+[àa]\s+m(a|es)\s+jour|me\s+faire\s+du\s+mal)\b', re.IGNORECASE),
]
