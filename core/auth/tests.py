...
def _mock_auth_celery_tasks():
    ...
    with patch("core.auth.views.send_login_notification_email") as m1, patch(
        "core.auth.views.send_password_changed_email"
    ) as m2:
        m1.delay = Mock()
        m2.delay = Mock()
        yield
...
class TestRegistration:
    ...
    def test_register_duplicate_display_name_case_insensitive(
...
class TestTwoFactor:
    ...
    def test_2fa_challenge_expired_token(self, client, tfa_user):
        ...
        assert resp.status_code == status.HTTP_400_BAD_REQUEST
        assert (
            "expired" in resp.data["error"].lower()
            or "invalid" in resp.data["error"].lower()
        )
    ...
    def test_2fa_backup_code(self, client, tfa_user):
        ...
        code_hash = hashlib.pbkdf2_hmac(
            "sha256", code.encode(), salt, iterations=100_000
        ).hex()
...
class TestEmailVerification:
    ...
    def test_resend_verification(self, client, create_user):
        ...
        with patch("core.auth.tasks.send_verification_email.delay") as mock_send:
            resp = client.post(
                "/api/auth/resend-verification/", {"email": "resend@test.com"}
            )
        assert resp.status_code == status.HTTP_200_OK
    ...
    def test_resend_verification_already_verified_silent(self, client, create_user):
        ...
        with patch("core.auth.tasks.send_verification_email.delay") as mock_send:
            resp = client.post(
                "/api/auth/resend-verification/", {"email": "already@test.com"}
            )
        assert resp.status_code == status.HTTP_200_OK
...
class TestPasswordReset:
    ...
    def test_request_reset_unknown_email_silent(self, client):
        ...
        with patch("core.auth.tasks.send_password_reset_email.delay") as mock_send:
            resp = client.post(
                "/api/auth/password/reset/", {"email": "nobody@test.com"}
            )
        assert resp.status_code == status.HTTP_200_OK
        mock_send.assert_not_called()
...
class TestAppleLogin:
    ...
    def test_apple_login_new_user(self, mock_verify, client):
        ...
        assert SocialAccount.objects.filter(
            user=user, provider="apple", uid="apple-uid-123"
        ).exists()
...
class TestFullFlows:
    ...
    def test_forgot_password_full_flow(self, client, create_user):
        ...
        with patch("core.auth.tasks.send_password_reset_email.delay"):
            resp = client.post(
                "/api/auth/password/reset/", {"email": "forgot@test.com"}
            )
        assert resp.status_code == status.HTTP_200_OK
