...
def log_auth_success(request, user):
    ...
    security_logger.info(
        "AUTH_SUCCESS ip=%s user_id=%s email=%s", ip, user.id, user.email
    )
...
def log_data_export(user, export_type="full"):
    ...
    security_logger.info(
        "DATA_EXPORT user_id=%s email=%s type=%s", user.id, user.email, export_type
    )
...
def log_jailbreak_attempt(request, text):
    ...
    security_logger.critical(
        "JAILBREAK_ATTEMPT ip=%s user_id=%s text=%.300s", ip, user_id, str(text)[:300]
    )
