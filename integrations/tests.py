...
class TestOpenAIService:
            ...
            'goals': [
                {
                    ...
                    'tasks': [
                        {'title': 'Task 1', 'order': 1, 'duration_mins': 30, 'description': 'Do it', 'reasoning': 'Because'}
                    ]
                }
            ],
            'tips': ['Tip 1'],
    ...
    def test_generate_calibration_questions(self, mock_openai):
        ...
        questions_json = json.dumps({
            ...
            'questions': [
                {'question': 'What is your experience level?', 'category': 'experience'},
                {'question': 'How many hours per week can you dedicate?', 'category': 'resources'},
            ]
        })
    ...
    def test_chat_stream_async(self):
        ...
        async def run_test():
            with patch('integrations.openai_service._async_client') as mock_async_client:
        ...
        async def run_test():
            with patch('integrations.openai_service._async_client') as mock_async_client:
                ...
                mock_async_create.return_value = Mock(
                    choices=[
                        Mock(message=Mock(content='Hello from async!'))
                    ],
