...
def _build_dream_identity(dream):
    ...
    if dream.target_date:
        ...
        if created:
            ...
            lines.append(
                f"Timeline: {elapsed} days elapsed of {total_days} total ({round(elapsed / max(1, total_days) * 100)}%)"
            )
...
def _build_calibration(dream):
    ...
    responses = CalibrationResponse.objects.filter(dream=dream).order_by(
        "question_number"
    )[:15]
...
def _build_checkin_history(dream):
    ...
    for i, ci in enumerate(checkins):
        ...
        recency = (
            ["most recent", "2nd most recent", "3rd most recent"][i]
            if i < 3
            else f"{i+1}th"
        )
...
def _build_obstacles(dream):
    ...
    for obs in obstacles:
        ...
        lines.append(
            f"- [{obs.obstacle_type}]{scope} {obs.title}: {obs.description[:150]}"
        )
        if obs.solution:
