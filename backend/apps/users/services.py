...
class BuddyMatchingService:
    ...
    def find_compatible_buddy(self, user: User) -> Optional[Tuple[User, float, List[str]]]:
        """
    ...
    def _get_eligible_candidates(self, user: User) -> List[User]:
        ...
        candidates = User.objects.filter(
            ...
            active_dreams_count__gte=1  # Must have at least one active dream
        )

        return list(candidates[:100])  # Limit to prevent performance issues

    def _calculate_compatibility(self, user1: User, user2: User) -> Tuple[float, List[str]]:
        """
        ...
        if user1_categories or user2_categories:
            ...
            category_score = len(shared_categories) / len(total_categories) if total_categories else 0
        else:
            category_score = 0
