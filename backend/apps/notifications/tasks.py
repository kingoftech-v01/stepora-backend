...
def process_pending_notifications(self):
    ...
    try:
        ...
        ).select_related('user').prefetch_related('user__fcm_tokens')
        ...
        for notification in pending:
            try:
                # Check if user wants notifications at this time (DND)
                if not fcm_service.should_send_notification(notification.user, now):
                ...
                # Send notification
                success = fcm_service.send_notification(notification)
                ...
                logger.error(f"FCM error for notification {notification.id}: {str(e)}")
                notification.status = 'failed'
                notification.save()
                failed_count += 1

            except Exception as e:
                logger.error(f"Unexpected error processing notification {notification.id}: {str(e)}")
                notification.status = 'failed'
                notification.save()
                failed_count += 1
