...
from apps.dreams.tasks import (
    generate_dream_skeleton_task, generate_initial_tasks_task, _calculate_pace,
)
...
def run_dream_flow(dream_config):
    ...
    for num, cat, question, answer in dream_config['calibration']:
        CalibrationResponse.objects.create(
            dream=dream, question_number=num, category=cat,
    ...
    try:
        ...
        print(f"  Skeleton: {ms_count} milestones, {goal_count} goals ({round(time.time()-t0,1)}s)")
    ...
    ).order_by('expected_date', 'order')[:5]
    completed = 0
    ...
    try:
        ...
        for q in questions:
            print(f"    [{q.get('id','')}] ({q.get('question_type','')}) {q.get('question','')[:90]}")

        # Language check: opening should be in the dream's language
        log_result(f"{label} questionnaire generated", len(questions) >= 3, f"{len(questions)} questions")

        # Check questions reference dream context (not generic)
        q_text_all = ' '.join(q.get('question', '') for q in questions).lower()
    ...
    try:
        ...
        for a in actions:
            print(f"    - {a.get('tool','?')}({', '.join(list(a.get('args',{}).keys())[:3])})")
        print(f"  Tasks: {tasks_before} -> {tasks_after} (+{tasks_after - tasks_before})")
        ...
        log_result(f"{label} pace_status returned", pace_status is not None, pace_status)
        log_result(f"{label} next_checkin_days returned", next_days is not None, str(next_days))
        log_result(f"{label} months not regressed",
                   months_through is None or months_through >= months_before,
                   f"{months_before} -> {months_through}")
        log_result(f"{label} coaching not empty", len(coaching) > 20, f"{len(coaching)} chars")
        log_result(f"{label} actions taken", len(actions) > 0, f"{len(actions)} actions")
...
user, created = User.objects.get_or_create(
    email='test_dual_dreams@dreamplanner.test',
...
dream_a_config = {
    ...
    'days': 365,
    'calibration': [
        (1, 'experience', "Quel est votre niveau actuel en musique?",
         "Aucun, je n'ai jamais joue d'un instrument."),
        (2, 'preferences', "Quel instrument vous interesse?",
         "Je sais pas trop, peut-etre la guitare ou le piano."),
        (3, 'resources', "Avez-vous un instrument?",
         "Non, rien du tout."),
        (4, 'motivation', "Pourquoi la musique?",
         "J'aime bien ecouter de la musique et je me dis que ce serait cool de jouer."),
        (5, 'constraints', "Des contraintes particulieres?",
...
dream_b_config = {
    ...
    'days': 365,
    'calibration': [
        (1, 'experience', "Do you have any business experience?",
         "Not really, I've only ever been an employee."),
        (2, 'preferences', "What kind of business are you thinking about?",
        ...
        (4, 'motivation', "Why do you want to start a business?",
         "I'm bored at my job and want more freedom."),
        (5, 'constraints', "Any constraints we should know about?",
...
if result_b:
    ...
    d.refresh_from_db()
    tc = Task.objects.filter(goal__dream=d).count()
    print(f"  Dream B '{d.title}': {tc} tasks, {d.progress_percentage}% progress, months={d.tasks_generated_through_month}")
