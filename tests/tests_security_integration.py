...
class TestDreamAISubscriptionGates:
    ...
    def test_premium_user_can_analyze_dream(self, mock_openai, premium_client, premium_user):
...
class TestOwnershipEnforcement:
    ...
    def test_user_cannot_update_other_profile(self, db):
        ...
        client = APIClient()
        client.force_authenticate(user=user2)
        response = client.patch(f'/api/users/{user1.id}/', {
...
class TestInputSanitization:
    ...
    def test_xss_in_profile_bio_sanitized(self, authenticated_client):
        response = authenticated_client.patch('/api/users/update-profile/', {
...
class TestNotificationTierFiltering:
    ...
    def test_free_user_does_not_see_buddy_notification(self, authenticated_client, user):
        from apps.notifications.models import Notification
        from django.utils import timezone as tz
        Notification.objects.create(
            user=user,
