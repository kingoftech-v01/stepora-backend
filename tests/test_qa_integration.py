...
from apps.users.models import Achievement, GamificationProfile, User, UserAchievement
...
class TestQA_TwoFactorLifecycle:
    ...
    def test_2fa_requires_authentication(self, db):
        ...
        assert (
            anon.post("/api/users/2fa/verify/", {"code": "123456"}).status_code == 401
        )
...
class TestQA_DreamLifecycle:
    ...
    def test_dream_full_lifecycle_with_idempotency(self, free_client, free_user):
        ...
        task1 = Task.objects.create(
            goal=goal, title="Task 1", order=0, duration_mins=30
        )
        task2 = Task.objects.create(
            goal=goal, title="Task 2", order=1, duration_mins=15
        )
...
class TestQA_DreamSharing:
    ...
    def test_share_and_access_flow(
        self, free_client, free_user, second_client_qa, second_user_qa
    ):
        ...
        r = free_client.post(
            f"/api/dreams/dreams/{dream_id}/share/",
            {
    ...
    def test_cannot_share_with_self(self, free_client, free_user):
        dream = Dream.objects.create(
            user=free_user, title="Self Share", status="active"
        )
        r = free_client.post(
...
class TestQA_BuddyLifecycle:
    ...
    def test_buddy_pairing_full_flow(
        self,
        premium_client_qa,
        premium_user_qa,
        second_premium_client_qa,
        second_premium_qa,
    ):
        # 1. Create pairing
        r = premium_client_qa.post(
            "/api/buddies/pair/",
            {
        ...
        r = premium_client_qa.post(
            f"/api/buddies/{pairing_id}/encourage/",
            {
...
class TestQA_AchievementNotification:
    ...
    def test_dream_completion_triggers_achievement_notification(self, free_user):
        ...
        dream = Dream.objects.create(
            user=free_user, title="Achieve Test", status="active"
        )
        dream.complete()
...
class TestQA_SearchAPI:
    ...
    def test_search_returns_categorized_results(
        self, mock_search, free_client, free_user
    ):
        dream = Dream.objects.create(
            user=free_user, title="Python Mastery", status="active"
        )
    ...
    def test_search_type_filter(self, mock_search, free_client, free_user):
        ...
        mock_search.assert_called_once_with(
            free_user, "test", types=["dreams"], limit=10
        )
