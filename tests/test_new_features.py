...
from apps.dreams.models import Dream, Goal, SharedDream, Task
...
class TestBackupCodes:
    ...
    def test_backup_code_can_verify(self, authenticated_client, user_with_2fa):
        ...
        # Get backup codes
        regen_resp = authenticated_client.post(
            "/api/users/2fa/backup-codes/",
            {
                "password": "testpassword123",
            },
...
class TestProgressRecalculationSignal:
    ...
    def test_task_deletion_recalculates_goal_progress(self, db, user):
        ...
        task1 = Task.objects.create(
            goal=goal, title="Task 1", order=0, status="completed"
        )
        task2 = Task.objects.create(
            goal=goal, title="Task 2", order=1, status="pending"
        )
        task3 = Task.objects.create(
            goal=goal, title="Task 3", order=2, status="pending"
        )
    ...
    def test_completed_task_deletion_recalculates(self, db, user):
        ...
        task2 = Task.objects.create(
            goal=goal, title="Task 2", order=1, status="completed"
        )
