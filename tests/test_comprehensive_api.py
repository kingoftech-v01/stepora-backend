...
class TestSubscriptionGatingComplete:
    ...
    def test_free_blocked_from_store_purchase(self, authenticated_client):
        response = authenticated_client.post(
            "/api/store/purchase/", {"item_id": str(uuid.uuid4())}
        )
        assert response.status_code == status.HTTP_403_FORBIDDEN

    def test_free_blocked_from_store_xp_purchase(self, authenticated_client):
        response = authenticated_client.post(
            "/api/store/purchase/xp/", {"item_id": str(uuid.uuid4())}
        )
        assert response.status_code == status.HTTP_403_FORBIDDEN
    ...
    def test_free_blocked_from_ai_calibration(self, authenticated_client, user):
        dream = Dream.objects.create(user=user, title="Test", status="active")
        response = authenticated_client.post(
            f"/api/dreams/dreams/{dream.id}/start_calibration/"
        )
        assert response.status_code == status.HTTP_403_FORBIDDEN
...
class TestOwnershipAndIDOR:
    ...
    def test_cannot_update_other_user_dream(self, second_client, user):
        ...
        response = second_client.patch(
            f"/api/dreams/dreams/{dream.id}/", {"title": "Hacked"}
        )
        assert response.status_code == status.HTTP_404_NOT_FOUND
    ...
    def test_cannot_update_other_user_profile(self, second_client, user):
        response = second_client.patch(
            f"/api/users/{user.id}/", {"display_name": "Hacked"}
        )
        assert response.status_code in (
    ...
    def test_goal_queryset_isolation(self, authenticated_client, user, second_user):
        ...
        other_dream = Dream.objects.create(
            user=second_user, title="Other", status="active"
        )
    ...
    def test_notification_queryset_isolation(
    ...
    def test_cannot_view_other_user_notifications_by_id(
...
class TestXSSInjection:
    ...
    @pytest.mark.parametrize("payload", XSS_PAYLOADS)
    def test_xss_in_conversation_message(
        self, premium_client, premium_user, mock_openai, payload
    ):
    ...
    def test_xss_in_tag_name(self, authenticated_client, user, payload):
        dream = Dream.objects.create(user=user, title="Test", status="active")
        response = authenticated_client.post(
            f"/api/dreams/dreams/{dream.id}/tags/",
            {
                "tag_name": payload,
            },
        )
...
class TestDreamsAPI:
    ...
    def test_share_dream(self, authenticated_client, dream, second_user):
        response = authenticated_client.post(
            f"/api/dreams/dreams/{dream.id}/share/",
            {
    ...
    def test_progress_history(self, authenticated_client, dream):
        response = authenticated_client.get(
            f"/api/dreams/dreams/{dream.id}/progress-history/"
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_update_task(self, authenticated_client, task):
        response = authenticated_client.patch(
            f"/api/dreams/tasks/{task.id}/", {"title": "Updated Task"}
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_resolve_obstacle(self, authenticated_client, dream):
        ...
        response = authenticated_client.post(
            f"/api/dreams/obstacles/{obstacle.id}/resolve/"
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_analyze_dream_premium(self, premium_client, premium_user, mock_openai):
        dream = Dream.objects.create(
            user=premium_user, title="Test Dream", status="active"
        )
    ...
    @pytest.mark.xfail(
        reason="DefaultRouter dream-detail pattern captures templates as pk"
    )
...
class TestSocialAPI:
    ...
    def test_accept_friend_request(
        self, authenticated_client, second_client, user, second_user
    ):
        ...
        if isinstance(results, list) and len(results) > 0:
            ...
            if request_id:
                accept_resp = authenticated_client.post(
                    f"/api/social/friends/accept/{request_id}/"
                )
    ...
    def test_reject_friend_request(
        ...
        second_client.post(
            "/api/social/friends/request/", {"targetUserId": str(user.id)}
        )
        pending = authenticated_client.get("/api/social/friends/requests/pending/")
        results = pending.data.get("results", pending.data)
        if isinstance(results, list) and len(results) > 0:
            ...
            if request_id:
                resp = authenticated_client.post(
                    f"/api/social/friends/reject/{request_id}/"
                )
    ...
    def test_unfollow_user(self, authenticated_client, second_user):
        authenticated_client.post(
            "/api/social/follow/", {"targetUserId": str(second_user.id)}
        )
        response = authenticated_client.delete(
            f"/api/social/unfollow/{second_user.id}/"
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_unblock_user(self, authenticated_client, second_user):
        authenticated_client.post(
            "/api/social/block/", {"targetUserId": str(second_user.id)}
        )
    ...
    def test_block_prevents_friend_request(
        self, authenticated_client, second_client, user, second_user
    ):
...
class TestIntegrationFlows:
    ...
    def test_social_friendship_flow(
        self, authenticated_client, second_client, user, second_user
    ):
        ...
        if isinstance(results, list) and len(results) > 0:
            ...
            resp = authenticated_client.delete(
                f"/api/social/friends/remove/{second_user.id}/"
            )
            assert resp.status_code == 200
    ...
    def test_blocking_removes_relationships(
        self, authenticated_client, second_client, user, second_user
    ):
...
class TestCirclesAPI:
    ...
    def test_circle_feed_requires_membership(
        self, pro_client, second_pro_client, pro_user
    ):
        resp = pro_client.post(
            "/api/circles/",
            {
...
class TestEdgeCases:
    ...
    def test_special_chars_in_search(self, authenticated_client):
        for char in ["%", "+", "&", "#", "=", "?"]:
            response = authenticated_client.get(
                "/api/social/users/search", {"q": f"test{char}user"}
            )
    ...
    def test_huge_page_size(self, authenticated_client):
        response = authenticated_client.get(
            "/api/dreams/dreams/", {"page_size": "99999"}
        )
        assert response.status_code in (status.HTTP_400_BAD_REQUEST, status.HTTP_200_OK)
    ...
    def test_api_nonexistent_endpoint(self, authenticated_client):
        ...
        response = authenticated_client.get(
            "/api/dreams/dreams/00000000-0000-0000-0000-000000000000/"
        )
        assert response.status_code == status.HTTP_404_NOT_FOUND
