...
check(
    "Unauth GET /api/users/me/ -> 401",
    resp.status_code == 401,
    f"got {resp.status_code}",
)

resp = client.post(
    "/api/auth/login/", {"email": "free@manual.com", "password": "TestPass123!"}
)
...
resp = client.post(
    "/api/auth/login/", {"email": "free@manual.com", "password": "wrong"}
)
check(
    "Login wrong password -> 400/401",
...
check(
    "Login nonexistent user -> 400/401",
    resp.status_code in (400, 401),
    f"got {resp.status_code}",
)
...
resp = client.patch(
    "/api/users/update_profile/", {"display_name": "Updated Name", "bio": "Hello world"}
)
check("PATCH update_profile -> 200", resp.status_code == 200, f"got {resp.status_code}")

resp = client.get("/api/users/stats/")
check(
    "GET /api/users/stats/ -> 200", resp.status_code == 200, f"got {resp.status_code}"
)
...
check(
    "GET /api/users/achievements/ -> 200",
    resp.status_code == 200,
    f"got {resp.status_code}",
)
...
check(
    "PUT notification-preferences -> 200",
    resp.status_code == 200,
    f"got {resp.status_code}",
)
...
check(
    "Free GET follow-suggestions -> 403",
    resp.status_code == 403,
    f"got {resp.status_code}",
)
...
check(
    "Free GET /notifications/ -> 200 (allowed)",
    resp.status_code == 200,
    f"got {resp.status_code}",
)
...
check(
    "Free GET /social/friends/ -> 200 (allowed)",
    resp.status_code == 200,
    f"got {resp.status_code}",
)
...
resp = client.patch(
    f"/api/dreams/dreams/{dream.id}/", {"title": "Learn Advanced Python"}
)
...
check(
    "POST duplicate dream -> 200/201",
    resp.status_code in (200, 201),
    f"got {resp.status_code}",
)

resp = client.post(
    f"/api/dreams/dreams/{dream.id}/share/", {"shared_with_id": str(second_user.id)}
)
check(
    "POST share dream -> 200/201",
    resp.status_code in (200, 201),
    f"got {resp.status_code}",
)
...
check(
    "Free 4th dream -> 403 (limit)", resp.status_code == 403, f"got {resp.status_code}"
)
...
check(
    "POST resolve obstacle -> 200", resp.status_code == 200, f"got {resp.status_code}"
)
...
check(
    "Other user PATCH dream -> 404", resp.status_code == 404, f"got {resp.status_code}"
)

resp = other_client.delete(f"/api/dreams/dreams/{active_dream.id}/")
check(
    "Other user DELETE dream -> 404", resp.status_code == 404, f"got {resp.status_code}"
)
...
check(
    "Premium GET /buddies/current/ -> 200",
    resp.status_code == 200,
    f"got {resp.status_code}",
)
...
check(
    "Premium GET /leagues/ -> 200", resp.status_code == 200, f"got {resp.status_code}"
)

resp = pclient.get("/api/circles/")
check(
    "Premium GET /circles/ -> 200", resp.status_code == 200, f"got {resp.status_code}"
)

resp = pclient.post("/api/circles/", {"name": "Test", "description": "x"})
check(
    "Premium POST /circles/ -> 403", resp.status_code == 403, f"got {resp.status_code}"
)
...
with patch("integrations.openai_service._client") as mock_oa:
    ...
    mock_chat.return_value = Mock(
        choices=[
            Mock(
                message=Mock(content="I can help plan your goals!", function_call=None)
            )
        ],
...
resp = client.post(
    "/api/social/friends/request/", {"targetUserId": str(premium_user.id)}
)
check(
    "Send friend request -> 200/201",
    resp.status_code in (200, 201),
    f"got {resp.status_code}",
)
...
check(
    "Follow user -> 200/201", resp.status_code in (200, 201), f"got {resp.status_code}"
)
...
check(
    "Report user -> 200/201", resp.status_code in (200, 201), f"got {resp.status_code}"
)

resp = client.post("/api/social/block/", {"targetUserId": str(second_user.id)})
check(
    "Block user -> 200/201", resp.status_code in (200, 201), f"got {resp.status_code}"
)
...
check(
    "GET notification detail -> 200", resp.status_code == 200, f"got {resp.status_code}"
)
...
check(
    "GET /store/categories/ (public) -> 200",
    resp.status_code == 200,
    f"got {resp.status_code}",
)
...
check(
    "Stripe webhook no sig -> 400", resp.status_code == 400, f"got {resp.status_code}"
)
...
check(
    "Pro POST /circles/ -> not 403", resp.status_code != 403, f"got {resp.status_code}"
)
...
for payload in xss_payloads:
    ...
    if resp.status_code in (200, 201):
        ...
        check(
            f'XSS "{payload[:30]}" sanitized in title',
            "<script" not in title.lower()
            and "onerror=" not in title.lower()
            and "onload=" not in title.lower(),
        )

resp = client.patch(
    "/api/users/update_profile/", {"bio": "<img src=x onerror=alert(1)>"}
)
if resp.status_code == 200:
...
for payload in sqli_payloads:
    ...
    check(
        f'SQLi "{payload[:30]}" in search -> not 500',
...
check(
    "SQLi in user search -> not 500", resp.status_code != 500, f"got {resp.status_code}"
)
...
with patch("apps.subscriptions.services.StripeService.create_customer"):
    edge_user = User.objects.create_user(
        email="edge@manual.com", password="TestPass123!", display_name="Edge User"
    )
...
resp = edge_client.post(
    "/api/dreams/dreams/",
    {"title": "", "description": "x", "category": "education", "priority": 1},
)
...
resp = edge_client.post(
    "/api/dreams/dreams/",
    {
        "title": "Apprendre le japonais 日本語",
        "description": "Unicode",
        "category": "education",
        "priority": 1,
    },
...
check(
    "Unicode in title -> 201", resp.status_code in (200, 201), f"got {resp.status_code}"
)
