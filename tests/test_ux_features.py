...
from apps.conversations.models import Conversation, Message, MessageReadStatus, Call
...
def _disable_es_signals():
    ...
    with _patch('django_elasticsearch_dsl.signals.RealTimeSignalProcessor.handle_save'), \
         _patch('django_elasticsearch_dsl.signals.RealTimeSignalProcessor.handle_delete'), \
         _patch('django_elasticsearch_dsl.signals.RealTimeSignalProcessor.handle_pre_delete'):
        yield
...
class TestBlockEnforcement:
    ...
    def test_send_message_blocked_returns_403(
        ...
        resp = client_a.post('/api/buddies/send-message/', {
    ...
    def test_call_blocked_returns_403(self, client_a, user_a, user_b):
        ...
        with patch('apps.notifications.fcm_service.FCMService'):
            resp = client_a.post('/api/conversations/calls/initiate/', {
...
class TestBuddyMessageNotification:
    ...
    def test_notification_created_on_send(
        ...
        notif = Notification.objects.filter(user=user_b, notification_type='buddy').first()
    ...
    def test_notification_body_truncated(
        ...
        long_msg = 'x' * 200
        client_a.post('/api/buddies/send-message/', {
            'conversationId': str(buddy_conversation.id),
            'content': long_msg,
        })
...
class TestUnreadCountAndMarkRead:
    ...
    def test_unread_count_in_conversation_list(
        ...
        assert resp.status_code == status.HTTP_200_OK
        convs = resp.data if isinstance(resp.data, list) else resp.data.get('results', resp.data)
...
class TestCallHistory:
    ...
    def test_history_returns_user_calls(self, client_a, user_a, user_b):
        ...
        call1 = Call.objects.create(
            caller=user_a, callee=user_b, call_type='voice', status='completed',
        )
        call2 = Call.objects.create(
            caller=user_b, callee=user_a, call_type='video', status='missed',
        )
    ...
    def test_history_excludes_other_users_calls(self, db, client_a, user_a, user_b):
        ...
        other = User.objects.create_user(
            ...
            display_name='Other', subscription='premium',
            subscription_ends=timezone.now() + timedelta(days=30),
        )
        Call.objects.create(
            caller=user_b, callee=other, call_type='voice', status='completed',
        )
