...
calibration_data = [
    ...
    (
        5,
        "constraints",
        "Y a-t-il des contraintes medicales ou physiques?",
...
obstacles_data = [
    (
        "Douleur au dos",
        "Les lombalgies pourraient limiter certains exercices",
        "predicted",
        "Eviter les exercices a fort impact, privilegier le renforcement du core",
    ),
    (
        "Grignotage a 16h",
        "Habitude de grignoter l'apres-midi au bureau",
        "predicted",
        "Preparer des snacks sains a l'avance (fruits secs, yaourt)",
    ),
    (
        "Timidite en salle",
        "Le user est timide et n'a jamais ete en salle",
        "predicted",
...
overdue_tasks = Task.objects.filter(goal__dream=dream, status="pending").order_by(
    "expected_date"
)[:2]
...
try:
    ...
    for q in questions:
        ...
        if q_type == "choice":
            ...
            print(
                f"      Scale: {q.get('scale_min', 1)}-{q.get('scale_max', 5)} {q.get('scale_labels', {})}"
            )
except Exception as e:
...
for q in questions:
    ...
    if q_id == "satisfaction":
        ...
        user_responses[q_id] = (
            "Mon dos me fait un peu mal apres les exercices, et j'ai du mal a resister au grignotage de 16h"
        )
        ...
        user_responses[q_id] = (
            "Ca va globalement, mais les exercices de dos sont difficiles."
        )
    else:
